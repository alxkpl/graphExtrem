...
$$
tr(\overline \Gamma \Theta)= \text{tr}(\overline \Gamma URU^t) + \sum_{k=1}^K(a_{kk} - r_{kk})\text{tr}(\overline \Gamma_{C_k})
$$
...
& = u_k^t\gamma(\Theta^+)u_l -\text{tr}\big[\gamma(\Theta^+)_{C_k}\big] p_k  -   \text{tr}\big[\gamma(\Theta^+)_{C_l}\big] p_l,
\end{align*}

with $u_k$ the k-th column of the matrix of clusters $U$. 
...
&= \text{tr}\big[\gamma(\Theta^+)_{C_k}\big] (1-p_k) + \frac 12 u_k^t\gamma(\Theta^+)u_k
\end{align*}


But all matrices obtained from the $\gamma$ application get only zeros in their diagonal so the final expression is : 

$$
\frac{\partial L_{log}(R)}{\partial r_{kl}} = \begin{cases} \frac 12 u_k^t\gamma(\Theta^+)u_k & \text{if } k=l, \\u_k^t\gamma(\Theta^+)u_l & \text{otherwise.}\end{cases} 
$$
...
$$
L_{trace}(R) = \text{tr}(\overline \Gamma URU^t)
$$
...
$$
\frac{\partial L_{trace}(R)}{\partial r_{kl}} = 2(U^t\overline \Gamma U)_{kl} 
$$

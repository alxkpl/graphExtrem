...
We have that $F(n)$ is equivalent to $\log(n)^{-4}$ when $n$ tends to infinity. Then, let's take a sequence $(\zeta_n)_{n \in \mathbb N^*}$ which is equivalent to $\log(n)^s$ with $0< s < 4$, for example $\zeta_n = \log(n)^3$. Then we have : 

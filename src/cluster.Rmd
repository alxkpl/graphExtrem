...
Thus, we know that if $\varepsilon_f$ is small, then $\Theta^* = f(R^*) \in \mathcal S_d^1$ and $\Theta$ are close for the Frobenius norm, so we finally have $\Theta^* \in \mathcal P_d^1$.

For the second point, we need to show first that : 

$$
\tilde D^2(r_{r \cdot}, r_{l \cdot}) \le \varepsilon_f \Longrightarrow ||f(R) - f(R^*)||_F \le ??? 
$$
...
\begin{align*}
| \frac12\text{tr} (\bar \Gamma \Theta) - \frac12\text{tr} (\bar \Gamma \tilde\Theta)| &= | \sum_{i,j} \bar \gamma_{ij}(\theta_{ij} - \tilde \theta_{ij}) | \\&\le  \sum_{i,j} |\bar \gamma_{ij}||\theta_{ij} - \tilde \theta_{ij}|\\ &\le (2 \sum_{i<j} |\bar \gamma_{ij}| )||\Theta - \tilde \Theta||_F
\end{align*}
...
$$
\lambda_k = \min_{\dim V = k-1} ||A\Pi_{V^\perp}||_2 
$$
...
**Lemma.** The generalised determinant application $\Theta \mapsto |\Theta|_+$ is Lipschitzian on $\bar B(\Theta, \varepsilon)$. 

**Proof.** Let $\tilde \Theta \in \bar B(\Theta, \varepsilon)$. 

$$
|\lambda_k(\tilde \Theta) - \lambda_k(\Theta) | \le \varepsilon \le 1
$$
And thus : 

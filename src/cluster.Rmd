...
where $\log(|\cdot|)$ is the logarithm of the determinant and $tr(\cdot)$ the trace.
...
D^2(\theta_{.i}, \theta_{.j}) =& \sum_{t\neq i,j} (\theta_{it} - \theta_{jt})^2 \\ 
 = &\sum_{m \neq k,l} p_m (r_{km}-r_{lm})^2 +(1-p_k) (r_{kk}-r_{lk})^2+(1-p_l) (r_{ll}-r_{lk})^2\\ 
 = & \tilde D^2(r_k, r_l).
\end{align*}
...
$$
\mathcal P(R) = \sum_{l<k} W_{lk} \tilde D^2(r_k, r_l)
$$

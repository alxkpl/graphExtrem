...
pacman::p_load(targets, graphicalExtremes)
```
...
- $\chi_{ijk} = 3-2\Phi(\sqrt{\Gamma_{ij}}/2)-2\Phi(\sqrt{\Gamma_{jk}}/2)-2\Phi(\sqrt{\Gamma_{ik}}/2)+ c_\Gamma$ where $\Phi$ is the distribution function of a standard Gaussian variable and $c_\Gamma = \sum$
